/**
 * LotChance — client-side Texas Lottery scraper.
 *
 * Runs in the phone/browser. Fetches CSV + HTML from the Texas Lottery website
 * via same-origin /proxy/* routes (Netlify redirects in the dist build,
 * Express routes in server.js locally), then parses everything here with the
 * browser's own DOMParser — no server-side logic.
 *
...
(function () {
    const PROXY = {
        csv:       '/proxy/csv',
        all:       '/proxy/all',
        closing:   '/proxy/details/closing.html',
        details:   (slug) => `/proxy/details/${slug}`,
        retailers: '/proxy/retailers',
        nominatim: (path) => `/proxy/nominatim/${path}`
    };
    ...
    async function fetchCSV() {
        const res = await fetch(PROXY.csv, { cache: 'no-store' });
        if (!res.ok) throw new Error(`CSV HTTP ${res.status}`);
        const text = await res.text();
        const lines = text.split('\n').filter((l) => l.trim());
        const data = lines.slice(2); // skip title + header
        const map = new Map();
    ...
    async function fetchCurrentGames() {
        const res = await fetch(PROXY.all, { cache: 'no-store' });
        if (!res.ok) throw new Error(`all.html HTTP ${res.status}`);
    ...
    async function fetchClosingInfo() {
        const res = await fetch(PROXY.closing, { cache: 'no-store' });
        if (!res.ok) throw new Error(`closing.html HTTP ${res.status}`);
        const doc = new DOMParser().parseFromString(await res.text(), 'text/html');
        const map = new Map();
    ...
    async function fetchGames() {
        ...
        return {
            ...
            source: 'Texas Lottery CSV (client-side)'
        };
    }
    ...
    async function fetchRetailers({ zip = '', city = '', gameNumber = '', limit = 50 } = {}) {
        ...
        params.append('zip', normalizeZip(zip));
        // Empty = all lottery retailers; specific number = only those that
        // have reported stocking that game.
        params.append('gameNumber', gameNumber ? String(gameNumber) : '');
        params.append('smoking', '');
        params.append('Submit', 'Search >>');

        // GET via the same-origin Netlify proxy. We previously POSTed, but
        // POST-through-redirect was unreliable from mobile PWAs (the body
        // sometimes failed to forward). The JSP form accepts GET equivalents
        // of the same params.
        ...
        const url = `${PROXY.retailers}?${query}`;
        console.log(`[Scraper] fetchRetailers: zip="${zip}" city="${city}" game="${gameNumber}"`);
        let res;
        try {
            res = await fetch(url, { method: 'GET', cache: 'no-store' });
        } catch (e) {
            console.error('[Scraper] retailers fetch failed:', e.message);
            throw e;
        }
        if (!res.ok) {
            console.error(`[Scraper] retailers HTTP ${res.status}`);
            throw new Error(`Retailers HTTP ${res.status}`);
        }
        ...
        console.log(`[Scraper] fetchRetailers parsed ${unique.length} retailers`);
    ...
    async function fetchOverallOdds(gameId, slug) {
        ...
        const res = await fetch(PROXY.details(slug), { cache: 'no-store' });
    ...
    async function fetchCityList() {
        ...
        const params = new URLSearchParams({ submitted: '', city: '', zip: '', gameNumber: '', smoking: '' });
        const res = await fetch(`${PROXY.retailers}?${params.toString()}`, { cache: 'no-store' });
        if (!res.ok) throw new Error(`city list HTTP ${res.status}`);
        const doc = new DOMParser().parseFromString(await res.text(), 'text/html');
