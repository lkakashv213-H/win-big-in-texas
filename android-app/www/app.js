...
class LotChanceApp {
    ...
    async fetchFromAPI() {
        try {
            console.log('[Scraper] Fetching live data on-device...');

            // Client-side scrape via Netlify CORS-pipe redirects (see scraper.js)
            const result = (typeof window.LotteryScraper !== 'undefined')
                ? { success: true, data: (await window.LotteryScraper.fetchGames()).games, meta: { fetchedAt: new Date().toISOString() } }
                : { success: false };
    ...
    async forceRefreshFromAPI() {
        try {
            console.log('[Scraper] Force refreshing on-device...');

            const result = (typeof window.LotteryScraper !== 'undefined')
                ? { success: true, data: (await window.LotteryScraper.fetchGames()).games }
                : { success: false };
    ...
    /**
     * Fetch JSON from the Nominatim geocoder with a valid User-Agent.
     * Nominatim's usage policy rejects requests without one and returns an
     * "Access denied" text/HTML body, which made the old bare fetch().json()
     * calls throw "Unexpected token ... is not valid JSON". On Android the
     * CapacitorHttp fetch-patch forwards this header to the native request; in a
     * plain browser the header is ignored and the UA the browser sends is fine —
     * safe in both environments. Parsing is tolerant so a non-JSON body surfaces
     * a clean error instead of a raw SyntaxError.
     */
    async nominatimJson(url) {
        const res = await fetch(url, { headers: { 'User-Agent': 'LotChance-Android/1.0' } });
        const text = await res.text();
        if (!res.ok) throw new Error(`geocoder HTTP ${res.status}`);
        try {
            return JSON.parse(text);
        } catch {
            throw new Error('geocoder returned a non-JSON response');
        }
    }
    ...
    async searchRetailersByLocation() {
        ...
        try {
            ...
            const geoResults = await this.nominatimJson(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&countrycodes=us&limit=1`);
    ...
    async fetchRealRetailers(lat, lng) {
        ...
        const geoData = await this.nominatimJson(
        ...
        const geocodePromises = result.data.slice(0, 15).map(async (retailer, index) => {
            ...
            try {
                ...
                const geocodeData = await this.nominatimJson(
    ...
    generateNearbyRetailers(lat, lng) {
        // Generate realistic-looking retailer data near the location
        // In production, this would come from Texas Lottery API or Google Places
        const retailerTypes = [
            { name: '7-Eleven', type: 'Convenience Store' },
            { name: 'Stripes', type: 'Convenience Store' },
            { name: 'Murphy USA', type: 'Gas Station' },
            { name: 'Valero', type: 'Gas Station' },
            { name: 'H-E-B', type: 'Grocery Store' },
            { name: 'Walmart', type: 'Supercenter' },
            { name: 'Kroger', type: 'Grocery Store' },
            { name: 'CVS Pharmacy', type: 'Pharmacy' },
            { name: 'Walgreens', type: 'Pharmacy' },
            { name: 'RaceTrac', type: 'Gas Station' },
            { name: 'QuikTrip', type: 'Convenience Store' },
            { name: 'Buc-ee\'s', type: 'Travel Center' }
        ];

        const retailers = [];
        const numRetailers = 8 + Math.floor(Math.random() * 5);

        for (let i = 0; i < numRetailers; i++) {
            const retailerType = retailerTypes[Math.floor(Math.random() * retailerTypes.length)];
            const offsetLat = (Math.random() - 0.5) * 0.05;
            const offsetLng = (Math.random() - 0.5) * 0.05;
            const distance = Math.sqrt(offsetLat * offsetLat + offsetLng * offsetLng) * 69;

            retailers.push({
                id: i + 1,
                name: retailerType.name,
                type: retailerType.type,
                address: `${Math.floor(Math.random() * 9999) + 1000} ${['Main St', 'Oak Ave', 'Highway 290', 'FM 1960', 'Westheimer Rd', 'I-35 Frontage'][Math.floor(Math.random() * 6)]}`,
                lat: lat + offsetLat,
                lng: lng + offsetLng,
                distance: t('retailer.milesAway', { d: distance.toFixed(1) }),
    ...
    async zipCentroid(zip) {
        ...
        try {
            // nominatimJson throws on a non-2xx. Letting that reach the catch
            // below (rather than caching a null) matters: caching would leave
            // the rest of the session with no distances at all because of one
            // throttled burst.
            const d = await this.nominatimJson(`https://nominatim.openstreetmap.org/search?format=json&postalcode=${zip}&country=us&limit=1`);
     ...
     */
    async runRetailerSearch({ lat, lng, zip = '', city = '', radiusMiles = null, silentOnEmpty = false }) {
        ...
        if (!zip && !city) {
            try {
                const d = await this.nominatimJson(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`);
    ...
    async refineRetailerDistances(retailers, lat, lng, maxRadius, token) {
        ...
        for (const r of retailers.slice(0, LotChanceApp.MAX_EXACT_GEOCODES)) {
            ...
            try {
                const d = await this.nominatimJson(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(full)}&countrycodes=us&limit=1`);
    ...
    /** Rough Texas bounding box — this is a Texas-only app, so a map center
     *  outside it (e.g. a garbage/stale Leaflet center after a programmatic
     *  setView) must never kick off a doomed retailer search. */
    isInTexas(lat, lng) {
        return lat >= 25.5 && lat <= 36.6 && lng >= -106.8 && lng <= -93.4;
    }

    maybeReSearchAfterMapMove() {
        if (!this._lastSearchCenter || !this._lastSearchRadius) return;
        const c = this.retailerMap.getCenter();
        // Ignore moves that land outside Texas (stale/garbage center, or the
        // user panned off-state) — searching there only yields a confusing
        // "none found" error that clobbers the current results.
        if (!this.isInTexas(c.lat, c.lng)) {
            console.log(`[Map] auto-research skipped — center (${c.lat.toFixed(2)}, ${c.lng.toFixed(2)}) is outside Texas`);
            return;
        }
        ...
        if (grew || shifted) {
            ...
            // Background re-search: stay silent if it finds nothing so it never
            // overwrites the current list with an error toast.
            this.runRetailerSearch({ lat: c.lat, lng: c.lng, radiusMiles: radius, silentOnEmpty: true });
        }
    }
    ...
    detectNearMeLocation() {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                try {
                    const data = await this.nominatimJson(
                        `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`
                    );
        ...
        for (const [key, group] of groups) {
            ...
            try {
                const q = group.zip
                    ...
                    : `${group.city}, Texas`;
                const results = await this.nominatimJson(
